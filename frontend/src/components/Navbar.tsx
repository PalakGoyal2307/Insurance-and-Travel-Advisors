            ...
            {!isAuthenticated ? (
              <>
                <button
                  ...
                  className="ml-2 px-4 py-2.5 text-[#0D2B5E] font-bold text-sm hover:text-[#F47B20] transition-colors rounded-xl hover:bg-blue-50"
                >
                  Login
                </button>
                <button
                  onClick={onRegisterClick}
                  className="px-4 py-2.5 rounded-full font-bold text-sm text-white transition-all duration-300 shadow-lg hover:shadow-xl hover:scale-105"
      ...
      {mobileOpen && (
        <div className="xl:hidden bg-white border-t border-blue-100 max-h-[80vh] overflow-y-auto">
          <div className="px-4 py-3 space-y-1">
            ...
            {!isAuthenticated ? (
              <>
                <button onClick={() => { setMobileOpen(false); onLoginClick?.() }} className="block w-full text-left py-3 px-4 text-[#0D2B5E] font-bold rounded-xl hover:bg-blue-50">Login</button>
                <button onClick={() => { setMobileOpen(false); onRegisterClick?.() }} className="block w-full py-3 px-4 rounded-xl font-bold text-white text-center" style={{ background: 'linear-gradient(135deg, #F47B20, #F0C060)' }}>Register</button>
              </>
            ) : (
              <>
                {!isAdmin && <button onClick={() => { setMobileOpen(false); onProfileClick?.() }} className="block w-full text-left py-3 px-4 text-[#0D2B5E] font-bold rounded-xl hover:bg-blue-50">Profile</button>}
                {isAdmin && <button onClick={() => { setMobileOpen(false); onAdminClick?.() }} className="block w-full text-left py-3 px-4 text-[#0D2B5E] font-bold rounded-xl hover:bg-blue-50">Admin Dashboard</button>}
                <button onClick={() => { setMobileOpen(false); onLogoutClick?.() }} className="block w-full py-3 px-4 rounded-xl font-bold text-white text-center" style={{ background: 'linear-gradient(135deg, #F47B20, #F0C060)' }}>Logout</button>
              </>
            )}
          </div>
        </div>
