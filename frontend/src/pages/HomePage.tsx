...
export default function HomePage({ navigate, queryContext, setQueryContext, currentUser }: Props) {
  ...
  const [destinationForm, setDestinationForm] = useState({ name: currentUser?.fullName || '', phone: currentUser?.phone || '', email: currentUser?.email || '', message: '' })
  ...
  const [contactForm, setContactForm] = useState({ name: currentUser?.fullName || '', phone: currentUser?.phone || '', email: currentUser?.email || '', message: '' })
  const [formSent, setFormSent] = useState(false)
  const [formError, setFormError] = useState('')

  useEffect(() => {
    if (!currentUser) return
    setDestinationForm((prev) => ({ ...prev, name: prev.name || currentUser.fullName, phone: prev.phone || currentUser.phone, email: prev.email || currentUser.email }))
    setContactForm((prev) => ({ ...prev, name: prev.name || currentUser.fullName, phone: prev.phone || currentUser.phone, email: prev.email || currentUser.email }))
  }, [currentUser])
  ...
  const handleContactSubmit = async (e: React.FormEvent) => {
    ...
    try {
      try {
        ...
        console.error('Backend contact storage failed', backendError)
      }
      ...
      setContactForm({ name: currentUser?.fullName || '', phone: currentUser?.phone || '', email: currentUser?.email || '', message: '' })
      setFormSent(true)
  ...
  const handleDestinationSubmit = async (e: React.FormEvent) => {
    ...
    try {
      try {
        ...
        console.error('Backend destination inquiry storage failed', backendError)
      }
      ...
      setDestinationForm({ name: currentUser?.fullName || '', phone: currentUser?.phone || '', email: currentUser?.email || '', message: '' })
...
>
              ...
              <div className="flex gap-3 flex-wrap">
                ...
                <a href="https://web.whatsapp.com/send?phone=919284476387" target="_blank" rel="noreferrer" className="flex items-center gap-2 px-5 py-3 rounded-2xl font-bold text-white text-sm bg-[#25D366] transition-all hover:scale-105 shadow-lg">
                  <svg
  className="w-5 h-5"
  fill="currentColor"
  viewBox="0 0 24 24"
      ...
      <footer style={{ background: 'linear-gradient(135deg, #050f1f 0%, #0D2B5E 100%)' }} className="py-10 px-6 text-center">
        <div className="max-w-5xl mx-auto">
          ...
          <div className="text-white/50 text-xs">
            © 2026 PNP Advisors. All rights reserved. | <a href={`https://mail.google.com/mail/?view=cm&fs=1&to=${CONTACT_EMAIL}`} className="hover:text-[#F47B20] underline">{CONTACT_EMAIL}</a>
          </div>
        </div>
      </footer>
    </div>
