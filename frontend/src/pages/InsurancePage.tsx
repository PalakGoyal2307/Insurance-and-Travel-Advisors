...
function HealthInsurancePage({ navigate, currentUser }: { navigate: (p: PageId) => void; currentUser: AuthUser | null }) {
  ...
  const popupMeta = popupType === 'health-quote'
    ? {
        title: 'Get Quote',
        sourceContext: 'insurance:popup:health-quote',
        insuranceType: 'Health Insurance',
        submitLabel: 'Submit Quote Request',
      }
    : {
        title: 'Free Consultation',
        sourceContext: 'insurance:popup:health-consultation',
        insuranceType: 'Health Insurance',
  ...
  const handleSubmit = async ({
    ...
    try {
      await createHealthApplication({
        ...
        additionalMembers,
        planName: planLabel,
        sourceContext: popupMeta.sourceContext,
      })
      ...
      const sentByEmailJs = await sendEmailWithAutoReply({
        ...
        ownerTemplateParams: {
          phone,
          context: popupMeta.sourceContext,
        ...
        autoReplyTemplateParams: {
          phone,
          context: popupMeta.insuranceType,
          inquiry_type: inquiryType,
          selected_plan: planLabel,
        },
      })
...
function LifeInsurancePage({ navigate, currentUser }: { navigate: (p: PageId) => void; currentUser: AuthUser | null }) {
  ...
  const popupMeta = popupType === 'life-quote'
    ? {
        title: 'Get Quote',
        sourceContext: 'insurance:popup:life-quote',
        insuranceType: 'Life Insurance',
        submitLabel: 'Submit Quote Request',
      }
    : {
        title: 'Book Free LIC Consultation',
        sourceContext: 'insurance:popup:life-consultation',
        insuranceType: 'Life Insurance',
        submitLabel: 'Submit Consultation Request',
      }
...
function GeneralInsurancePage({ navigate, currentUser }: { navigate: (p: PageId) => void; currentUser: AuthUser | null }) {
  ...
  const openPopup = async (type: GeneralPopupType, planName = '') => {
    if (!currentUser) {
      navigate('login')
      return
    }
  ...
  const popupMeta = popupType === 'general-enquiry'
    ? {
        title: 'Enquire Now',
        sourceContext: 'insurance:popup:general-enquiry',
        insuranceType: 'General Insurance',
        submitLabel: 'Submit Enquiry',
      }
    : {
        title: 'Free Insurance Audit',
        sourceContext: 'insurance:popup:general-audit',
        insuranceType: 'General Insurance',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const fullMessage = [
      `Inquiry Type: ${inquiryType}`,
      `Insurance Type: ${popupMeta.insuranceType}`,
      `Selected Plan: ${planLabel}`,
      `Full Name: ${fullName}`,
    ...
    try {
      try {
        ...
        console.error('Backend general application creation failed', backendError)
      }
      ...
      setFormSent(true)
      setForm({ fullName: currentUser?.fullName || '', phone: currentUser?.phone || '', email: currentUser?.email || '', primaryAddress: '', primaryPincode: '', city: '', businessType: '', coverageType: '', requirements: '' })
      setTimeout(() => {
        closePopup()
      }, 1400)
    } catch (error) {
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4" onClick={closePopup}>
          ...
          <div className="relative bg-white rounded-3xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto" onClick={e => e.stopPropagation()}>
            ...
            <form onSubmit={handleSubmit} className="p-6 sm:p-8 space-y-4">
              ...
              <RequirementChecklist requirements={requirements.length ? requirements : APPLICATION_DOCUMENTS.general.map(item => ({ ...item, status: 'required' as const, document: null }))} scope="general" onRefresh={loadRequirements} />
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <input value={form.primaryAddress} onChange={e => updateForm('primaryAddress', e.target.value)} className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm" placeholder="Primary Member Address*" required />
                <input value={form.primaryPincode} onChange={e => updateForm('primaryPincode', e.target.value.replace(/\D/g, '').slice(0, 6))} className="w-full px-4 py-3 rounded-xl border border-gray-200 text-sm" placeholder="Primary Member Pincode*" inputMode="numeric" pattern="\d{6}" required />
...
export default function InsurancePage({ page, navigate, openQueryForm, currentUser }: Props) {
